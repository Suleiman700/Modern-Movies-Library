  ...
  return (
    <React.Fragment>
      ...
      <Menu isOpen={isOpened} onStateChange={isMenuOpen} styles={styles}>
        <LanguageSelection />
        <Heading>{Languages.getSelectedLanguageTranslation('discover')}</Heading>
        {renderStatic(staticCategories, selected, setisOpened)}
        <Heading>{Languages.getSelectedLanguageTranslation('genres')}</Heading>
        ...
        <CopyRight mobile={true}>
          Copyright ©
          <StyledLink href="https://github.com/Suleiman700" target="_blank">
            Suleiman
          </StyledLink>
        </CopyRight>
        <Svg
...
function renderStatic(categories, selected, setisOpened) {
  return categories.map((categoryName, i) => (
    <LinkWrap
      to={`${process.env.PUBLIC_URL}/discover/${categoryName}`}
      key={i}
    ...
    >
      <MenuItem
        ...
        // title={category}
        title={Languages.getSelectedLanguageTranslations()[categoryName]}
        selected={categoryName === selected ? true : false}
      />
    </LinkWrap>
  ));
}
